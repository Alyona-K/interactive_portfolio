...
-------------------------------------------------- */
export const animateContactBg = (section: HTMLElement | null, bg: HTMLElement | null) => {
...
const mobileScene = (bg: HTMLElement) => {
  gsap.set(bg, { clearProps: "all", width: "100%", borderRadius: "0px", xPercent: -50 });
};
...
const tabletScene = (bg: HTMLElement) => {
  gsap.set(bg, { clearProps: "all", width: "100%", borderRadius: "30px", xPercent: -50 });
  ...
  const update = () => {
    ...
    const tExpand = Math.min(Math.max(1 - (top - expandEnd) / (expandStart - expandEnd), 0), 1);
    const tShrink = Math.min(Math.max(1 - (bottom - shrinkEnd) / (shrinkStart - shrinkEnd), 0), 1);
...
const desktopScene = (bg: HTMLElement) => {
  gsap.set(bg, { clearProps: "all", width: "70%", borderRadius: "30px", xPercent: -50 });
...
//   // стартовые стили
//   gsap.set(bg, { width: "70%", borderRadius: "30px", xPercent: -50 });

//   const update = () => {
//     const rect = bg.getBoundingClientRect(); // считаем верхнюю и нижнюю границы фона
//     const viewportHeight = window.innerHeight;

//     const top = rect.top; // верх фона относительно viewport
//     const bottom = rect.bottom; // низ фона

//     let width, borderRadius;

//     // === Фаза 1: расширение, верхняя граница только появилась ===
//     const expandStart = viewportHeight; // верх фона только выходит снизу
//     const expandEnd = viewportHeight / 2 - 180; // центр экрана
//     let tExpand = 1 - (top - expandEnd) / (expandStart - expandEnd);
//     tExpand = Math.min(Math.max(tExpand, 0), 1);

//     // === Фаза 2: сжатие, нижняя граница коснулась верха ===
//     const shrinkStart = viewportHeight / 2 + 180; // центр экрана
//     const shrinkEnd = 0; // верх экрана
//     let tShrink = 1 - (bottom - shrinkEnd) / (shrinkStart - shrinkEnd);
//     tShrink = Math.min(Math.max(tShrink, 0), 1);
